...
function readXmlConfiguration() {
    ...
    $.ajax({
        ...
        success: function (xmlFile) {
            //save the xmlFile's data
            xmlData = xmlFile;
            var loadConfig = document.getElementById("loadConfigurations");
            ...
            $(xmlData).find("configuracion").each(function (i) {
                var nombre = $(this).attr('nombre');
                var nave = $(this).find('nave').text();
                var modo = $(this).find('modoDificil').text();
...
function loadConfig() {
    ...
    //Finds the selected configuration's name in the xmlData passed to the server
    $(xmlData).find("configuracion").each(function () {
        var nombre = $(this).attr("nombre");
        if (nombre === configName) {
            //Forces a boolean return
            modoDificil = $(this).find("modoDificil").text().indexOf('true') > -1;
            colorNave = $(this).find("nave").text().toLowerCase();
        }
    });
}
